...
typedef struct {
    ...
    int pontoInteresse[V + 1]; // código do ponto (ex: 1=hotel, 2=restaurante)
...
void caminhoPontoInteresse(GRAFO *g, int origem, int x, int n) {
    ...
    while (!filaVazia(f)) {
        ...
        for (int j = 1; j <= V; j++) {
            if (g->matriz[atual][j] == 1 && g->flag[j] == 0 && j != n) {
                g->flag[j] = 1;
...
int main() {
    ...
    // Busca do hospital (3), a partir de 1, evitando o local 5
    caminhoPontoInteresse(&g, 1, 3, 5);

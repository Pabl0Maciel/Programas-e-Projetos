...
void exibirPilhaInvertida(PILHA* pilha){
    printf("Pilha Invertida (%d), base: ", tamanho_pilha(pilha));
    int final = pilha -> topo - (tamanho_pilha(pilha)-1);
    for (int i = final; i < tamanho_pilha(pilha); i++){
        printf("%d ", pilha -> elementos[i].chave);
    }
    printf("topo\n");
}

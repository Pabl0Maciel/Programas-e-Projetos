...
int encontrarMax(DEQUE *deque, int *max) {
    int resposta = -1;
    
    if (deque -> cabeca -> prox == deque -> cabeca) return resposta;

    ELEMENTO *atual = deque -> cabeca -> prox;
    *max = atual -> reg.chave;

    while (atual != deque -> cabeca){
        if (*max < atual -> prox -> reg.chave){
            *max = atual -> prox -> reg.chave;
        }
        atual = atual -> prox;
    }
...
int main(){
    ...
    int *max;
    int maximo = encontrarMax(&deque, &max);
    printf("Maior chave do deque: %d\n", maximo);
